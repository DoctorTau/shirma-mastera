<script lang="ts">
	...
	async function submit(e: Event) {
		...
		try {
			...
			error = err instanceof ApiError ? err.message : 'Не удалось зарегистрироваться';
		} finally {
			loading = false;
		}
	}
...
<div class="gr-page login-screen">
	<form onsubmit={submit}>
		...
		<input type="password" bind:value={password} placeholder="Пароль (мин. 8 символов)" />
		{#if error}
			<p class="error">{error}</p>
		{/if}
		<button type="submit" disabled={loading || !email || !password}>Зарегистрироваться</button>
